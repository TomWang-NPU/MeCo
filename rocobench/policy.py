...
class PlannedPathPolicy:
    ...
    def __init__(
        ...
        #print(f"path_plan内容1: {path_plan}")
        ...
        if self.parse_llm_plan_to_qpos(physics, path_plan, update=True) is None:
    ...
    def parse_llm_plan_to_qpos(
        ...
        #print(f"joint_qpos_target内容: {joint_qpos_target}")
    ...
    def plan_qpos(self, physics):
        ...
        #print(f"start_qpos内容: {start_qpos}")
        ...
        #         camera_id='teaser', height=4000, width=6000,
        #     )
        ...
        #     #     )], axis=1)
        #     plt.imshow(img_arr)
        #     plt.savefig(f"a_pack_5_{i}.png", format="png", dpi=300)
        #     i += 1
        #     # plt.show()
        #
        #     # dist = np.linalg.norm(
        ...
        #print(f"goal_qpos内容: {self.joints_qpos_target}")
        if path[0] is None:
    ...
    def planner(self, env, q):

        physics = env.physics
        ee_pose = self.rrt_planner.forward_kinematics_all(
            q=q,
            physics=physics,
            return_4=True
        )
    ...
    def collision(self, env, q):

        physics = env.physics
        bad = self.rrt_planner.check_collision(
            robot_qpos=q,
            physics=physics,
            allow_grasp=True,
            check_grasp_ids=self.grasp_allowed,
