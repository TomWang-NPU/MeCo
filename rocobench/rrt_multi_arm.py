...
class MultiArmRRT:
    ...
    def forward_kinematics_all(
        ...
        if return_4:
            ee_poses_quat = {}
            for robot_name, pose in ee_poses.items():
                #quat = euler_to_quat(pose.orientation, ordering='XYZ')  # 欧拉角转换成四元数
                combined = np.concatenate([pose.position, pose.orientation])  # 将 position 和 quat 合并成同一形状
                ee_poses_quat[robot_name] = combined  # 存入 NumPy 数组
            return ee_poses_quat
