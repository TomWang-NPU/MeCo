...
""" 201
fixed_positions = {
    "apple": np.array([-0.25 ,  0.4  ,  0.3]),
    "banana": np.array([-0.25,  0.6 ,  0.25]),
    "milk": np.array([-0.5 ,  0.4,  0.45]),
    "soda_can": np.array([-0.5,  0.6,  0.33]),
    "bread": np.array([-0.75,  0.4,  0.27]),
    "cereal": np.array([-0.75,  0.6,  0.55]),
}
"""

fixed_positions = {
    "apple": np.array([-0.5 ,  0.5  ,  0.3]),
    "banana": np.array([-0.05,  0.6 ,  0.25]),
    "milk": np.array([-0.5 ,  0.25,  0.45]),
    "soda_can": np.array([-0.5,  0.8,  0.33]),
    "bread": np.array([-0.75,  0.4,  0.27]),
    "cereal": np.array([-0.75,  0.6,  0.55]),
}
...
class PackGroceryTask(MujocoSimEnv):
    ...
    def sample_initial_scene(self): 
        ...
        a = self.seed_value
        # a = 0
        self.random_state = np.random.RandomState(a)
        ...
        for _idx, item_name in zip(panel_idxs, self.item_names):
            """
            if item_name in fixed_positions:
                #new_pos = np.array([fixed_positions[item_name][0], fixed_positions[item_name][1],fixed_positions[item_name][2]])
                new_pos = np.array([fixed_positions[item_name][0], fixed_positions[item_name][1],self.physics.data.body(item_name).xpos[2]])
            """
    ...
    def describe_obs_nosd(self, obs: EnvState):
        full_desp = ""
        for name in self.item_names:
            full_desp += self.describe_object(obs, name) + "\n"
...
"""
    ...
    def get_agent_prompt_agent(self, obs, agent_name):
        robot_name = self.get_robot_name(agent_name)
        other_robot = "Alice" if agent_name == "Bob" else "Bob"
        object_desp = "\n".join([self.describe_object(obs, name) for name in self.item_names])
    ...
    def get_agent_prompt_again(self, obs, agent_name,central_response):
        robot_name = self.get_robot_name(agent_name)
        other_robot = "Alice" if agent_name == "Bob" else "Bob"
        object_desp = "\n".join([self.describe_object(obs, name) for name in self.item_names])
...
Never forget you are {agent_name}!
Propose exactly one action for yourself at the **current** round, select from [Action Options].
The final plan for you must strictly follow [Action Output Instruction] and [Path Plan Instruction].
The central planner\'s current action plan is: 
{central_response}


Please evaluate the given plan. If you agree with it, respond 'I Agree', without any extra words. If not, briefly explain your objections to the central planner.
    """
        return agent_prompt
