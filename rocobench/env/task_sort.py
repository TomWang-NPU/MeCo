...
# EXECUTE\nNAME Alice ACTION WAIT\nNAME Bob ACTION PICK blue_square PLACE panel3\nNAME Chad WAIT
    ...
    def get_agent_prompt_again(self, obs, agent_name, central_response, include_response_instructions=True):
            cube_name, bin_name = self.cube_targets[agent_name]
            other_robots = ", ".join(
                [r for r in self.robots.keys() if r != agent_name]
            )
    ...
    """
            ...
            agent_prompt +=f"""The central planner\'s current action plan is: {central_response}
    ...
    def sample_initial_scene(self):
        ...
        far_panels = dict()
        far_panels['square'] = [i for i, tup in enumerate(tosample_panels)]
        far_panels['polygon'] = [i for i, tup in enumerate(tosample_panels)]
        far_panels['trapezoid'] = [i for i, tup in enumerate(tosample_panels)]
