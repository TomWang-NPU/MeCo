...
class MakeSandwichTask(MujocoSimEnv):
    ...
    def sample_initial_scene(self):
        ...
        #     self.random_state = np.random.RandomState(self.sed)
        # else:
        ...
        self.random_state.shuffle(sampled_panels)
        ...
        # 直接定义固定位置，按照需求调整
        fixed_positions = {
            "bread_slice1": np.array([-0.8, 0.6, 0.2]),
            "bacon": np.array([0.8, 0.6, 0.2]),
            "cheese": np.array([-0.4, 0.5, 0.2]),
            "tomato": np.array([0.6, 0.35, 0.2]),
            "cucumber": np.array([1.2, 0.7, 0.4]),
            "ham": np.array([-0.7, 0.3, 0.2]),
            "beef_patty": np.array([0.5, 0.7, 0.2]),
            "bread_slice2": np.array([-0.4, 0.3, 0.2]),
        }
        
        # 设定食材的固定位置
        for food in self.food_items:
            if food in fixed_positions:
                new_pos = fixed_positions[food]
            else:
                # 如果食材未在 fixed_positions 中定义，默认放在中心
                new_pos = np.array([0.0, 0.0, 0.2])

            new_quat = None  # 保持默认方向
            
            self.reset_body_pose(body_name=food, pos=new_pos, quat=new_quat)
            self.reset_qpos(jnt_name=f"{food}_joint", pos=new_pos, quat=new_quat)
        '''
...
End your response by either: 1) output PROCEED, if the plans require further discussion; 2) If everyone has made proposals and got approved, output the final plan, must strictly follow [Action Output Instruction]!
    ...
    def get_agent_prompt_again(self, obs, agent_name, central_plan) -> str:
            assert agent_name in self.agent_names, f"Agent {agent_name} is not in the scene"
            other_robot = [r for r in self.agent_names if r != agent_name][0]
            table_side = "left" if agent_name == "Dave" else "right"
            other_side = "right" if agent_name == "Dave" else "left"
    ...
    End your response by either: 1) output PROCEED, if the plans require further discussion; 2) If everyone has made proposals and got approved, output the final plan, must strictly follow [Action Output Instruction]!
            """
            agent_prompt += f"""The central planner\'s current action plan is: {central_plan}
