...
from prompting import LLMResponseParser, FeedbackManager, HAMSPrompter, RoCoPrompter, SingleThreadPrompter, ReActPrompter, save_episode_html
import time
...
class LLMRunner:
    def __init__(
        ...
        elif llm_comm_mode in ["ReAct"]:
            logging.warning(f'Using ReActPrompter for {llm_comm_mode} mode')
            self.prompter = ReActPrompter(
                env=self.env,
                parser=self.parser,
                feedback_manager=self.feedback_manager,
                max_tokens=1024,
        ...
        elif llm_comm_mode in ["HAMS"]:
            logging.warning(f'Using HAMS for {llm_comm_mode} mode')
            self.prompter = HAMSPrompter(
                env=self.env,
                parser=self.parser,
                feedback_manager=self.feedback_manager,
                max_tokens=1024,
    ...
    def one_run(self, run_id: int = 0, start_step: int = 0, skip_reset = False, prev_llm_plans = [], prev_response = None, prev_actions = None):
        ...
        for step in range(start_step, start_step + self.max_runner_steps):
            ...
            elif self.llm_comm_mode in ["HAMS"]:
    ...
    def run(self, args):
        ...
        if args.cont:
            ...
            load_run = glob(os.path.join(self.data_dir, args.load_run_name, f"re_run_{args.load_run_id}"))
            if len(load_run) == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_name", "-rn", type=str, default="data")
    ...
    parser.add_argument("--comm_mode", type=str, default="plan", choices=["Central", "RoCo", "HAMS", "ReAct"])
    ...
    parser.add_argument("--load_run_name", "-lr", type=str, default="run")
    ...
    parser.add_argument("--llm_source", "-llm", type=str, default="deepseek-v3")
    logging.basicConfig(level=logging.INFO)
