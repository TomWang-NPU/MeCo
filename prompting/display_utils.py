...
def save_episode_html(
    ...
    for step, step_dir in enumerate(step_dirs):
        ...
        for json_file in json_files:
            ...
            for di in _data:
                print(f"Unexpected type in _data: {type(di)}. Content: {di}")
            filter_data = []
            for di in _data:
                if isinstance(di, dict):
                    # 如果是字典，按正常流程获取 'sender' 并检查是否在 sender_keys 中
                    if di.get("sender", None) in sender_keys:
                        filter_data.append(di)
                elif isinstance(di, str):
                    # 如果是字符串类型，尝试解析为字典
                    try:
                        di_dict = json.loads(di)  # 尝试将 JSON 字符串转换为字典
                        if di_dict.get("sender", None) in sender_keys:
                            filter_data.append(di_dict)
                    except json.JSONDecodeError:
                        # 如果解析失败，打印提示信息
                        print(f"Failed to decode JSON from string: {di}")
                else:
                    # 非字典且非字符串的情况
                    print(f"Unexpected type in _data: {type(di)}. Content: {di}")
